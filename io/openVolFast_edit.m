function [header, BScanHeader, slo, BScans] = openVolFast_edit(path, options)
...
header.Eye = char(fread( fid, 4, '*uchar' )');
...
if(~any(strcmp(options, 'header')))
    BScans=zeros(header.SizeZ, header.NumAScans ,header.NumBScans, 'single');
else
    BScans= [];
end
...
BScanHeader.ILM = zeros(header.NumBScans,header.NumAScans, 'single');
BScanHeader.BM = zeros(header.NumBScans,header.NumAScans, 'single');
BScanHeader.NFL_GCL = zeros(header.NumBScans,header.NumAScans, 'single');

% ------------- MODIFICATION DAVID -------------------
BScanHeader.OffSeg = zeros(1,header.NumAScans, 'single');
BScanHeader.GCL_IPL = zeros(header.NumBScans,header.NumAScans, 'single');
BScanHeader.IPL_INL = zeros(header.NumBScans,header.NumAScans, 'single');
BScanHeader.INL_OPL = zeros(header.NumBScans,header.NumAScans, 'single');
BScanHeader.OPL_ONL = zeros(header.NumBScans,header.NumAScans, 'single');
BScanHeader.ELM = zeros(header.NumBScans,header.NumAScans, 'single');
BScanHeader.MZ_EZ = zeros(header.NumBScans,header.NumAScans, 'single');
BScanHeader.PHROS_IDZ = zeros(header.NumBScans,header.NumAScans, 'single');
BScanHeader.IDZ_RPE = zeros(header.NumBScans,header.NumAScans, 'single');
%----------------------------------------------    
...
for zz = 1:header.NumBScans
    ...
    status = fseek( fid, 16+2048+(header.SizeXSlo*header.SizeYSlo)+(ii*(header.BScanHdrSize+header.NumAScans*header.SizeZ*4)), -1 );
    ...
    % ------------- MODIFICATION DAVID -------------------
    BScanHeader.OffSeg(zz) = OffSeg; % just in case it is useful
    %----------------------------------------------    
...
%----------------------------------------------------------------------
    ...
    if(~any(strcmp(options, 'header')))
        status = fseek( fid, header.BScanHdrSize+2048+(header.SizeXSlo*header.SizeYSlo)+(ii*(header.BScanHdrSize+header.NumAScans*header.SizeZ*4)), -1 );
        oct = fread( fid, header.NumAScans*header.SizeZ, '*float32' );
        oct = reshape( oct, header.NumAScans, header.SizeZ );
        BScans(:,:,zz)=oct';
    end
    ...
    status = fseek( fid, 256+2048+(header.SizeXSlo*header.SizeYSlo)+(ii*(header.BScanHdrSize+header.NumAScans*header.SizeZ*4)), -1 );
    seg = (fread( fid, NumSeg*header.NumAScans, '*float' ))';
    
    BScanHeader.ILM(zz,:) = seg(1:header.NumAScans);
    BScanHeader.BM(zz,:) = seg((1:header.NumAScans) + header.NumAScans);
    BScanHeader.NFL_GCL(zz,:) = seg((1:header.NumAScans) + 2*header.NumAScans);
    
    if NumSeg > 3
        %--------------- MODIFICATION ------------------------
        BScanHeader.GCL_IPL(zz,:) = seg((1:header.NumAScans) + 3*header.NumAScans);
        BScanHeader.IPL_INL(zz,:) = seg((1:header.NumAScans) + 4*header.NumAScans);
        BScanHeader.INL_OPL(zz,:) = seg((1:header.NumAScans) + 5*header.NumAScans);
        BScanHeader.OPL_ONL(zz,:) = seg((1:header.NumAScans) + 6*header.NumAScans);   

        BScanHeader.ELM(zz,:) = seg((1:header.NumAScans) + 8*header.NumAScans);

        BScanHeader.MZ_EZ(zz,:) = seg((1:header.NumAScans) + 14*header.NumAScans);
        BScanHeader.PHROS_IDZ(zz,:) = seg((1:header.NumAScans) + 15*header.NumAScans);
        BScanHeader.IDZ_RPE(zz,:) = seg((1:header.NumAScans) + 16*header.NumAScans);
...
%     if NumSeg == 3
%         BScanHeader.NFL(zz,:) = seg(header.NumAScans*2+1:header.NumAScans*3);
%     end
%     figure()
%     imshow(BScans(:,:,zz).^0.25)    
%     title(num2str(zz))
%     for cR=1:NumSeg
%         hold on
%         curSeg=seg(header.NumAScans*(cR-1)+(1:header.NumAScans));
%         plot(curSeg,'r'); hold on; pause();
%     end
...
% BScanHeader.BM(BScanHeader.RPE>1e6) = nan;
% if NumSeg == 3
%     BScanHeader.NFL_GCL(BScanHeader.NFL>1e6) = nan;
% end
...
if visuseg == 1,
    ...
    hold on
    mesh(double(BScanHeader.BM));
    if NumSeg == 3
        hold on
        mesh(double(BScanHeader.NFL_GCL));
    end
end
...
%         metaWriteHelper(fidW, 'Eye', header.Eye, 'str');
...
%         metaWriteHelper(fidW, 'OctSize', [header.NumAScans header.SizeZ header.NumBScans]);
%         metaWriteHelper(fidW, 'OctScale', [header.ScaleX header.ScaleZ header.ScaleY]);
