...
% -------------- MODIFICATION -------------------
BScanHeader.GCL = zeros(header.NumBScans,header.SizeX, 'single');
BScanHeader.IPL = zeros(header.NumBScans,header.SizeX, 'single');
BScanHeader.INL = zeros(header.NumBScans,header.SizeX, 'single');
BScanHeader.OPL = zeros(header.NumBScans,header.SizeX, 'single');
BScanHeader.ELM = zeros(header.NumBScans,header.SizeX, 'single');
BScanHeader.ISOS = zeros(header.NumBScans,header.SizeX, 'single');
BScanHeader.IDZ = zeros(header.NumBScans,header.SizeX, 'single');
BScanHeader.RPEM = zeros(header.NumBScans,header.SizeX, 'single');
%----------------------------------------------    
...
%     BScans = BScans.^0.25;
    ...
    BScanHeader.RPE(zz,:) = seg((1:header.SizeX) + header.SizeX);
    BScanHeader.NFL(zz,:) = seg((1:header.SizeX) + 2*header.SizeX);
    
    if NumSeg > 3
        %--------------- MODIFICATION ------------------------
        BScanHeader.GCL(zz,:) = seg((1:header.SizeX) + 3*header.SizeX);
        BScanHeader.IPL(zz,:) = seg((1:header.SizeX) + 4*header.SizeX);
        BScanHeader.INL(zz,:) = seg((1:header.SizeX) + 5*header.SizeX);
        BScanHeader.OPL(zz,:) = seg((1:header.SizeX) + 6*header.SizeX);   

        BScanHeader.ELM(zz,:) = seg((1:header.SizeX) + 8*header.SizeX);

        BScanHeader.ISOS(zz,:) = seg((1:header.SizeX) + 14*header.SizeX);
        BScanHeader.IDZ(zz,:) = seg((1:header.SizeX) + 15*header.SizeX);
        BScanHeader.RPEM(zz,:) = seg((1:header.SizeX) + 16*header.SizeX);
...
%     for cR=1:NumSeg
%         hold on
%         curSeg=seg(header.SizeX*(cR-1)+(1:header.SizeX));
